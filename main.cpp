...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	//PSOを作成する
	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStageDesc{};
	graphicsPipelineStageDesc.pRootSignature = rootSignature;  //RootSignature
	graphicsPipelineStageDesc.InputLayout = inputLayoutDesc;  //InputLayout
	graphicsPipelineStageDesc.VS = { vertexShaderBlob->GetBufferPointer(),
	vertexShaderBlob->GetBufferSize() };  //VertexShader
	graphicsPipelineStageDesc.PS = { pixelShaderBlob->GetBufferPointer(),
	pixelShaderBlob->GetBufferSize() };  //PixelShader
	graphicsPipelineStageDesc.BlendState = blendDesc;  //BlendState
	graphicsPipelineStageDesc.RasterizerState = rasterizerDesc;  //ResterizerState
	//書き込むRTVの情報
	graphicsPipelineStageDesc.NumRenderTargets = 1;
	graphicsPipelineStageDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;
	//利用するトロポジのタイプ
	graphicsPipelineStageDesc.PrimitiveTopologyType =
		D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;
	//どのように画面に色を打ち込むかの設定
	graphicsPipelineStageDesc.SampleDesc.Count = 1;
	graphicsPipelineStageDesc.SampleMask = D3D12_DEFAULT_SAMPLE_MASK;
	//実際に生成
	ID3D12PipelineState* graphicsPipelineState = nullptr;
	hr = device->CreateGraphicsPipelineState(&graphicsPipelineStageDesc,
